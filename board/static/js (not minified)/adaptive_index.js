...
window.addEventListener('scroll', ()=>{
  ...
  /*---------------------------------- ОБРЕЗАЕМ ВОЗРАСТ -------------------------------------------------------*/
  $('.adt_age').each(function(i, obj) {
    if (obj.textContent.length >=8){
      var truncated = obj.textContent.replace('Возраст:', '').replace('Вік:', '');
      $(obj).text(truncated);
    }
  });
...
$( document ).ready(function() {
  if(window.location.href.includes('/uk/')){
    ...
    $( ".adt_content" ).before( "<span class='adt_content_header'>Описание:</span>" );
  }
  $(document).on("click", ".adt_more_info", function(e){ 
    $(this).children('.adt_info').css('display', 'block');
