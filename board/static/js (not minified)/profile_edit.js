...
$(document).on('click', '.delete_btn', function () {
  $(this).closest('.profile_adt').find('.pop_delete_adt').css('display', 'flex');
});
$(document).on('click', '.close_pop_delete_adt', function () {
  $(this).closest('.profile_adt').find('.pop_delete_adt').css('display', 'none');
});
...
$(document).ready(function () {
  if($('.archive')){ // проверяем наличие элемента
    ...
    if(window.location.href.includes('/uk/')){
      ...
      $('.archive .archive_btn a').text('Вернуть');
    }
  }
  if($('.edited')){ // проверяем наличие элемента
    ...
    if(window.location.href.includes('/uk/')){
      ...
      $( ".edited .adt_a").after( "<span class='edited_adt'>Скоро появится на сайте</span>" );
    }
  }
});