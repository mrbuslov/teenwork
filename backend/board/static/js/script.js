        ...
        $.ajax({
            ...
            success: function(data) {
                if(data === 'username_taken'){
                    document.getElementById('username_error_text').innerHTML = 'Нік зайнятий, будь ласка, придумайте інший'
                }
                else if(username.includes(' ')){
                    document.getElementById('username_error_text').innerHTML = 'Нік має бути без пробілів'
                }
                else if(username.length < 4 || username.length > 25){
                    document.getElementById('username_error_text').innerHTML = 'Нік повинен бути > 4 і < 25 символів'
                }
                else if(!username_regex){
                    document.getElementById('username_error_text').innerHTML = 'Нік повинен містити латинські літери'
                }
                else{
  ...
  $( "#phone_number" ).blur(function() {
      ...
      if(phone_number !== ''){                
          $.ajax({
              ...
              success: function(data) {
                  if(data === 'phone_taken'){
                      document.getElementById('phone_number_error_text').innerHTML = 'Здається, такий телефон ми десь бачили -_-'
                  }
                  else if(!/^\+?3?8?(0\d{9})$/.exec(phone_number)){
                      document.getElementById('phone_number_error_text').innerHTML = 'Перевірте ще раз, будь ласка'
                  }
                  else{
  ...
  $( "#email" ).blur(function() {
      ...
      if(email !== ''){                
          $.ajax({
              ...
              success: function(data) {
                  if(data === 'email_taken'){
                      document.getElementById('email_error_text').innerHTML = 'Здається, такий email ми десь бачили -_-'
                  }
                  else if(!/^\S+@\S+\.\S+$/.exec(email)){
                      document.getElementById('email_error_text').innerHTML = 'Перевірте ще раз, будь ласка'
                  }
                  else{
      ...
      if(password !== ''){          
        if(document.getElementById('password_error_text')){      
          if(password.length < 4){
            document.getElementById('password_error_text').innerHTML = 'Ми не хочемо, щоб Вас взламали, допишіть пару букв'
          }
          else{
        ...
        <div class="container">\
          ...
          <div class="container_info">\
                <span class="adt_name"> <span class="adt_n">\
                  <a href="'+data.fields.adt_url+'" class="adt_a">'+data.fields.title+'</a>'
            ...
            <span class="adt_created">'+data.fields.published+'\
              <a href="'+data.fields.fav_url+'" class="heart">';
  ...
  $.ajax({
    ...
    success: function(res) {
      ...
      if(response.success !== false && response.data[0] !== undefined && response.data[0].TotalCount !== 0){
        ...
        var searching_selections = '<ul class="city_all_results">';
        for(i in searching){
          searching_selections += `<li><p class="city_result_p">${searching[i].Present}</p><input type="hidden" value="${searching[i].MainDescription}" class="short_city_hidden"></li>`
        }
        searching_selections += '</ul>';
        $('#city_results').html(searching_selections);
      }
      else{ 
        $('#city_results').hide();
      }
...
$(document).on("click", ".city_result_p", function () {
  ...
  // $('#city_input').val($(this).parent().find('.short_city_hidden').val());
  $('#city_results').hide();
